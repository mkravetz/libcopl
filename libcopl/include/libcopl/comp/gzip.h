...
*/
#ifndef _LIB_LIBCOPL_COMP_GZIP_H
#define _LIB_LIBCOPL_COMP_GZIP_H

#include <libcopl/comp.h>

/**
 * @ingroup COMP
 * @defgroup GZIP Gzip algorithm setup routines for DC coprocessor
 * @{
 */

/**
 * @brief Setup gzip operation control blocks
 *
 * This routine provides a convenient mechanism to setup the control blocks
 * passed to the Decompression/Compression (DC) coprocessor for a gzip
 * operation.
 *
 ...
 */
#define cop_gzip_setup(crb, ctx, src, s_size, tgt, t_size) \
	cop_comp_setup(crb, ctx, src, s_size, tgt, t_size, COP_FF_GZIP)

/**
 * @brief Setup gunzip operation control blocks if only a single CRB is needed
 *
 * This routine provides a convenient mechanism to setup the control blocks
 * passed to the Decompression/Compression (DC) coprocessor for a gunzip
 ...
 * @return zero if successful, non-zero on failure
 */
#define cop_gunzip_setup_only(crb, ctx, src, s_size, tgt, t_size) \
	cop_decomp_setup_fo(crb, ctx, src, s_size, tgt, t_size, \
			    COP_FF_GZIP, COP_CHAINING_ONLY)

/**
 * @brief Setup gunzip operation control blocks for first CRB in a sequence
 *
 ...
 * sequence of CRBS used to perform a gunzip operation.
 *
 * @param crb pointer to CRB structure with pointers to CSB already set
 * @param ctx pointer to a previously populated cop_ctx structure assoiated
 *            with an open instance of the DC coprocessor
 ...
 * @return zero if successful, non-zero on failure
 */
#define cop_gunzip_setup_first(crb, ctx, src, s_size, tgt, t_size) \
	cop_decomp_setup_fo(crb, ctx, src, s_size, tgt, t_size, \
			    COP_FF_GZIP, COP_CHAINING_FIRST)

/**
 * @brief Setup gunzip operation control blocks for a middle CRB in a sequence
 *
 ...
 * @return zero if successful, non-zero on failure
 */
#define cop_gunzip_setup_middle(crb, src, s_size, tgt, t_size) \
	cop_decomp_setup_ml(crb, src, s_size, tgt, t_size, \
			    COP_CHAINING_MIDDLE)

/**
 * @brief Setup gunzip operation control blocks for a last CRB in a sequence
 *
 ...
 * @return zero if successful, non-zero on failure
 */
#define cop_gunzip_setup_last(crb, src, s_size, tgt, t_size) \
	cop_decomp_setup_ml(crb, src, s_size, tgt, t_size, \
			    COP_CHAINING_LAST)

/**
 * @brief Setup control blocks to add target buffer and resume gunzip operation
 *
 * This routine provides a convenient mechanism to setup control blocks that
 * will add a target buffer to a gunzip sequence already in progress
 ...
 * @return zero if successful, non-zero on failure
 */
#define cop_gunzip_addtgt_rsm_setup(crb, tgt, t_size) \
	cop_decomp_addtgt_rsm_setup(crb, tgt, t_size, COP_FF_GZIP)

/** @} */
#endif /* _LIB_LIBCOPL_COMP_GZIP_H */
