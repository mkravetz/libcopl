 ...
 * (DES) functions. Demonstrates how to use the low level interfaces. 
...
#define BN_PRINT(name) {\
	printf(#name " = ");\
	BN_print_fp(stdout, name);\
	printf("\n");\
}
	...
	unsigned char *data)
{
	...
	BN_pseudo_rand(BN_key, MAX_KEYSIZE * 8, -1, 0);
...
#define TEST_DES_ECB(name, evp, enc) {\
...
#define TEST_DES_CBC(name, evp, enc) {\
	struct cop_sym_ ## name ## _cpb *cpb = cop_get_cpbptr(crb);\
	do_sw_cipher( evp(), data, data_len, ciphertext_sw, &ciphertext_sw_sz, key, iv, enc);\
	cop_ ## name ## _setup(crb, &ctx, data, data_len, ciphertext_hw, data_len,\
		enc, COP_RPAD_NONE, 0x00);\
...
int main(int argc, char *argv[])
{
	 ...
	 * case will be 128 bytes since DES-CBC-256 has the largest CPB */
	size_t cb_size =
		sizeof(struct cop_crb) + sizeof(struct cop_csb) +
		sizeof(struct cop_sym_des_ecb_cpb);
	...
	for (i = 0; i < NUM_TESTS; i++) {
		...
		TEST_DES_ECB(des_ecb, EVP_des_ecb, 1);
		TEST_DES_ECB(tdes_ecb, EVP_des_ede3, 0);

		TEST_DES_CBC(des_cbc, EVP_des_cbc, 1);
		TEST_DES_CBC(tdes_cbc, EVP_des_ede3_cbc, 0);
	}
