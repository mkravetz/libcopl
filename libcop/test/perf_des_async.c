...
void work(void *args){
	...
	switch(TYPE){
	  case 1: TESTECB(cop_des_ecb_async);break;
	  case 2: TESTCBC(cop_des_cbc_async);break;
	  case 3: TESTECB(cop_tdes_ecb_async);break;
	  case 4: TESTCBC(cop_tdes_cbc_async);break;
...
{
	if(argc != 5){
	  printf("Usage: %s   TYPE(1=des_ecb, 2=des_cbc, 3=tdes_ecb, 4=tdes_cbc)   PKT_SIZE_BYTES   NUM_THREADS   NUM_TESTS_PER_THREAD\n", argv[0]);
	  return 0;
	} 
	else{
	  TYPE =  atoi(argv[1]);
	  ...
	  if(TYPE!=1 && TYPE!=2 && TYPE!=3 && TYPE!=4){
	    printf("TYPE can only be 1, 2, 3 or 4\n");
	    return(0);
	  }
	}
