...
static unsigned long startPMUCounter(int count)
{
  ...
  switch(count){
    case 0: return 0;
    case 1: 
      strcpy(EventCodeStr1,"WSP_CRYPTO_CRB_GOOD_ARC4");
      strcpy(EventCodeStr2,"PM_COMP_CRYPTO_PBIC_255");
      break;
    case 2: 
      strcpy(EventCodeStr1,"WSP_CRYPTO_MARKED_CRB_ACT_ARC4");
      strcpy(EventCodeStr2,"WSP_CRYPTO_MARKED_CRB_RCV_ARC4");
      break;
    case 3: 
      strcpy(EventCodeStr1,"WSP_CRYPTO_IDLE_ENG_ARC4_6");
...
#define TEST(type) {\
	for (i = 0; i < NUM_TESTS_PER_THREAD; i++) {\
		...
		if( type(key, indata_len, indata, outdata, NULL, NULL, NULL, COP_FLAG_ONESHOT)){\
...
void work(void *args){
	...
	switch(TYPE){
	  case 1: TEST(cop_rc4_40); break;
	  case 2: TEST(cop_rc4_64); break;
	  case 3: TEST(cop_rc4_128); break;
...
{
	if(argc != 6){
	  printf("Usage: %s   TYPE(1=arc4_40, 2=arc4_64, 3=arc4_128)   PKT_SIZE_BYTES   NUM_THREADS   NUM_TESTS_PER_THREAD COUNT(0=none, 1=Throughput 2=latency 3=engine utilization)\n", argv[0]);
	  return 0;
	} 
	else{
	  TYPE =  atoi(argv[1]);
