...
int main(int argc, char *argv[])
{
	...
	for (i=0; i< NUMINTERATIONS ; i++) {
		...
		tgt_uncompressed =(char *)cop_malloc(cc_pool, size +(size/16), 1);
...
if (gDD_version == 1) {
        ...
        if( errno ) { 
        	printf("gzip Comp Error: %d size %d output %d\n", errno,(int)size,(int)compressed_size);
			fail++; 
			errno=0;
		}
    ...
    // now decompress and see if we get the same thing
    	...
    	if( errno ) { 
        	printf("gzip decomp Error: %d size %d output %d\n", errno,(int)size,(int)uncompressed_size);
			fail++; 
			errno=0;
		}
