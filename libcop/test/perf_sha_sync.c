...
#define TESTHMAC(type){\
	for (i = 0; i < NUM_TESTS_PER_THREAD; i++) {\
	  generate_test_data(indata,key);\
	  if(type(indata, indata_len, outdigest, NULL, key, COP_FLAG_ONESHOT)){\ 
	    printf(": call failed in thread %d\n", p->id);\ 
	    p->fail++;\
	  }\
	  p->total++;\
...
void work(void *args){
	...
	switch(TYPE){
	  case 1: TEST(cop_sha1);break;
	  case 2: TEST(cop_sha256);break;
	  case 3: TEST(cop_sha512);break;
	  case 4: TESTHMAC(cop_sha1_hmac);break;
	  case 5: TESTHMAC(cop_sha256_hmac);break;
	  case 6: TESTHMAC(cop_sha512_hmac);break;
